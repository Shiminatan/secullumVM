<?php
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $data = $_GET['search'];
        $sql_total = "SELECT COUNT(*) FROM idcloud 
                      WHERE revenda LIKE '%$data%' 
                         OR cliente LIKE '%$data%' 
                         OR cnpj LIKE '%$data%' 
                         OR case_ LIKE '%$data%' 
        ...
        $total_registros = $conexao->query("SELECT COUNT(*) FROM idcloud")->fetch_row()[0];
    }
    ...
    $sql_ativos_count = "SELECT COUNT(*) as total FROM idcloud WHERE estado = 'ativo' AND (revenda LIKE '%$data%' OR cliente LIKE '%$data%' OR cnpj LIKE '%$data%' OR case_ LIKE '%$data%' OR banco LIKE '%$data%' OR login_ LIKE '%$data%' OR nserial LIKE '%$data%')";
    ...
    $sql_cancelados_count = "SELECT COUNT(*) as total FROM idcloud WHERE estado = 'cancelado' AND (revenda LIKE '%$data%' OR cliente LIKE '%$data%' OR cnpj LIKE '%$data%' OR case_ LIKE '%$data%' OR banco LIKE '%$data%' OR login_ LIKE '%$data%' OR nserial LIKE '%$data%')";
    ...
    $sql_a_cancelar_count = "SELECT COUNT(*) as total FROM idcloud WHERE estado = 'a cancelar' AND (revenda LIKE '%$data%' OR cliente LIKE '%$data%' OR cnpj LIKE '%$data%' OR case_ LIKE '%$data%' OR banco LIKE '%$data%' OR login_ LIKE '%$data%' OR nserial LIKE '%$data%')";
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark fixed-top" style="background-color: #999da1;">
        <a class="navbar-brand" href="#">ID CLOUD</a>
        <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <li class="nav-item">
                 ...
                 <li class="nav-item">
                    <a class="nav-link" href="#" onclick="verifyPassword()">
                        <button type="type" class="btn btn-primary btn-custom">LICENÇA FACIAL</button>
                    </a>
                </li>
                <li class="nav-item">
            ...
            <div class="d-flex align-items-center">
                <span class="h5 text-white me-3">Bem vindo <?php echo htmlspecialchars($logado, ENT_QUOTES, 'UTF-8'); ?></span>
                <?php
                // Verifica se o usuário é admin para mostrar os botões de cadastro
                if (isset($currentUser) && $currentUser['funcao'] == 'admin') {
                    echo '<a href="cadastroMassaIdcloud.php" class="btn btn-primary me-3">CADASTRAR</a>';
                }
                ?>
    ...
    <div class="box-search">
        ...
        <form action="excelIdcloud.php" method="post">
        ...
        <table class="table table-striped table-sm table-hover" style="text-align:left">
            ...
            <tbody>
                ...
                while ($user_data = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td>" . $user_data['rep'] . "</td>";
                    echo "<td>" . $user_data['nserial'] . "</td>";
                    echo "<td>" . $user_data['portaria'] . "</td>";
                    echo "<td>" . $user_data['sistema'] . "</td>";
                    echo "<td>" . $user_data['login_'] . "</td>";
                    //echo "<td>" . $user_data['senha'] . "</td>";
                    echo "<td>" . $user_data['data_incl'] . "</td>";
                    echo "<td>" . $user_data['ticket_incl'] . "</td>";
                    echo "<td>" . $user_data['case_'] . "</td>";
                    echo "<td>" . $user_data['data_canc'] . "</td>";
                    echo "<td>" . $user_data['ticket_canc'] . "</td>";
                    if ($currentUser['funcao'] == 'admin') {
                        echo "<td>
                             <a class='btn btn-primary btn-sm' href='editIdcloud.php?id=$user_data[id]'>
                                <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-pencil' viewBox='0 0 16 16'>
                                    <path d='M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325'/>
                                </svg>
                             </a>
                            <a class='btn btn-danger btn-sm' href='deleteIdcloud.php?id=$user_data[id]' onclick='return confirmDelete()'>
    ...
    <script>
        ...
        function searchData() {
            window.location = 'idcloud.php?search=' + search.value;
        }
