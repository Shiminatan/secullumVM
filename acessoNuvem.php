<?php
    ...
    // Verificar se a consulta foi bem-sucedida e se encontrou informaÃ§Ãµes do usuÃ¡rio
    if ($resultUserInfo->num_rows > 0) {
      $userInfo = $resultUserInfo->fetch_assoc();
      // Definir $currentUser com as info do usuÃ¡rio atualmente logado , vai olhar para o campo (funÃ§Ã£o) que esta informado no if pela linha 185
      $currentUser = array(
        'id' => $userInfo['id'],
        'nome' => $userInfo['nome'],
      ...
      // Se nÃ£o encontrar informaÃ§Ãµes do usuÃ¡rio, posso $currentUser como vazio ou null, dependendo da sua lÃ³gica de tratamento de erro
      $currentUser = null;
    }
    ...
    // Recuperar o nÃºmero da pÃ¡gina atual
    $pagina = isset($_GET['pagina']) ? (int)$_GET['pagina'] : 1;
    $pagina = ($pagina < 1) ? 1 : $pagina;
    $offset = ($pagina - 1) * $itens_por_pagina;

    // Consulta SQL ajustada para a paginaÃ§Ã£o
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $data = $_GET['search'];
        $sql = "SELECT * FROM acesso_nuvem
                WHERE revenda LIKE '%$data%' 
                   OR cliente LIKE '%$data%' 
                   OR cnpj LIKE '%$data%' 
                   OR link_acesso LIKE '%$data%' 
                   OR case_ LIKE '%$data%' 
                   OR nome_vm LIKE '%$data%' 
                   OR port_servidor LIKE '%$data%'			   
                ORDER BY id DESC 
                LIMIT $offset, $itens_por_pagina";
    } else {
        $sql = "SELECT * FROM acesso_nuvem
                ORDER BY nome_vm, port_servidor DESC
                LIMIT $offset, $itens_por_pagina";
    }
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $data = $_GET['search'];
        $sql_total = "SELECT COUNT(*) FROM acesso_nuvem 
                      WHERE revenda LIKE '%$data%' 
                         OR cliente LIKE '%$data%' 
                         OR cnpj LIKE '%$data%' 
                         OR link_acesso LIKE '%$data%' 
        ...
        $total_registros = $conexao->query("SELECT COUNT(*) FROM acesso_nuvem")->fetch_row()[0];
    }

    // Calcula o total de pÃ¡ginas
    $total_paginas = ceil($total_registros / $itens_por_pagina);

    //mostra os totais de ativos que sÃ£o trazidos pela consulta search 
    $sql_ativos_count = "SELECT COUNT(*) as total FROM acesso_nuvem WHERE estado = 'ativo' AND (revenda LIKE '%$data%' OR cliente LIKE '%$data%' OR cnpj LIKE '%$data%' OR link_acesso LIKE '%$data%' OR case_ LIKE '%$data%' OR nome_vm LIKE '%$data%' OR port_servidor LIKE '%$data%')";
    ...
    // Contabiliza todos os cancelados da tabela com base nos resultados da consulta principal
    $sql_cancelados_count = "SELECT COUNT(*) as total FROM acesso_nuvem WHERE estado = 'cancelado' AND (revenda LIKE '%$data%' OR cliente LIKE '%$data%' OR cnpj LIKE '%$data%' OR link_acesso LIKE '%$data%' OR case_ LIKE '%$data%' OR nome_vm LIKE '%$data%' OR port_servidor LIKE '%$data%')";
    ...
    // Contabiliza todos a cancelar da tabela com base nos resultados da consulta principal
    $sql_a_cancelar_count = "SELECT COUNT(*) as total FROM acesso_nuvem WHERE estado = 'a cancelar' AND (revenda LIKE '%$data%' OR cliente LIKE '%$data%' OR cnpj LIKE '%$data%' OR link_acesso LIKE '%$data%' OR case_ LIKE '%$data%' OR nome_vm LIKE '%$data%' OR port_servidor LIKE '%$data%')";
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark fixed-top" style="background-color: #999da1;">
        <a class="navbar-brand" href="#"> ACESSO NUVEM</a>
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto">
                <li class="nav-item">
                    <a class="nav-link" href="comunicadorServidor.php">
                        <button type="button" class="btn btn-primary btn-custom">COMUNICADOR SERVIDOR</button>
                    </a>
                </li>
                <li class="nav-item">
...
<div style="display: flex; align-items: center; margin-right: 15px;">
                ...
                <span class="h5 text-white" style="margin-right: 15px;">Bem vindo <?php echo htmlspecialchars($logado, ENT_QUOTES, 'UTF-8'); ?></span>
                
                <?php
                if (isset($currentUser) && $currentUser['funcao'] == 'admin') {
                    echo '<a href="cadastroMassaAcesso.php" class="btn btn-primary" style="margin-right: 15px;">CADASTRAR</a>';
                }
                ?>
    ...
    <div class="box-search">
        ...
        <form action="excelAcesso.php" method="post">
        ...
        <table class="table table-striped table-sm table-hover" style="text-align:left">
            <thead>
                <tr>
                    ...
                    <th scope="col">COD. EQUIP</th>
                    <th scope="col">NOME EQUIP</th>
                    <th scope="col">PORT SERVIDOR</th>
                    <th scope="col">PORT ONLINE</th>
                    ...
                    <th scope="col">MOBUSS</th>
                    <th scope="col">DATA INCL</th>
            ...
            <tbody>
                ...
                while ($user_data = mysqli_fetch_assoc($result)) {
                    ...
                    $estado = strtolower($user_data['estado']);
                    $cor = ($estado == 'cancelado') ? 'red' : (($estado == 'ativo') ? 'green' : 'black');
                    echo "<td><span style='color: $cor; font-weight: bold;'>" . $user_data['estado'] . "</span></td>";
                    echo "<td>" . $user_data['equip_mod'] . "</td>";
                    echo "<td>" . $user_data['equip_cod'] . "</td>";
                    echo "<td>" . $user_data['equip_nome'] . "</td>";
                    echo "<td>" . $user_data['port_servidor'] . "</td>";
                    echo "<td>" . $user_data['port_online'] . "</td>";
                    echo "<td>" . $user_data['push'] . "</td>";
                    echo "<td>" . $user_data['ip_servidor'] . "</td>";
                    echo "<td>" . $user_data['nome_vm'] . "</td>";
                    echo "<td>" . $user_data['link_acesso'] . "</td>";
                    echo "<td>" . $user_data['ticket_canc'] . "</td>";
                    echo "<td>" . $user_data['case_'] . "</td>";
                    echo "<td>" . $user_data['validade'] . "</td>";
                    echo "<td>" . $user_data['backup_'] . "</td>";
                    echo "<td>" . $user_data['mobuss'] . "</td>";
                    echo "<td>" . $user_data['data_incl'] . "</td>";
                    echo "<td>" . $user_data['data_canc'] . "</td>";
                    if ($currentUser['funcao'] == 'admin') {
                        echo "<td>
                            <a class='btn btn-primary btn-sm' href='editAcesso.php?id=$user_data[id]'>
                                <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-pencil' viewBox='0 0 16 16'>
                                    <path d='M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325'/>
                                </svg>
                            </a>
                            <a class='btn btn-danger btn-sm' href='deleteAcesso.php?id=$user_data[id]' onclick='return confirmDelete()'>
    ...
    <script>
        ...
        function searchData() {
            window.location = 'acessoNuvem.php?search=' + search.value;
        }
